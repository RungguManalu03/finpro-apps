...
@section('js')
        ...
        $(document).ready(function() {
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                Swal.fire("Berhasil!", "Pembayaran telah disetujui.",
                                    "success");
                                $("#user-list").DataTable().ajax
            ...
            $(document).on("click", ".btn-reject", function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/reject-payment/${id}`,
                            type: "GET",
                            success: function(response) {
                                Swal.fire("Berhasil!", "Pembayaran telah ditolak.",
                                    "success");
                                $("#user-list").DataTable().ajax
                                    .reload(); // Reload DataTable
                            },
    ...
    <script>
        ...
        let kost_id = pathArray[pathArray.length - 1];
        let user_datatable = $("#user-list").DataTable({
            processing: true,
            serverSide: true,
            // ajax: `{{ route('find-data-payment-admin') }}`,
            ajax: `{{ route('find-data-payment-admin') }}?kost_id=${kost_id}`,
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
                    orderable: false,
        ...
        $("#formTambahUser").on('submit', function(e) {
            ...
            let id = currentUrl.split("/").pop(); // ambil angka 1
